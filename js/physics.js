...
function newBall() {
    ...
    return {
        x: W / 2, y: 50, r: BALL_NORMAL_R,
        dx: dir * spd * 0.65, dy: spd,
        isMini: false, pierce: false, homing: false, trail: []
    };
}

function spawnDebrisParticles(bx, by, color) {
...
function debrisToMiniball(d) {
    ...
    balls.push({
        x: d.x, y: paddle.y - 5, r: BALL_NORMAL_R,
        dx: hit * spd * .8 + (Math.random() - .5),
        dy: -(spd + Math.random()),
        isMini: false, pierce: bonusIs('pierce'), homing: false, trail: []
    });
}
...
function updateBall(b) {
    ...
    if (b.dy > 0 && b.x > paddle.x && b.x < paddle.x + paddle.w &&
        ...
        // якщо homing — гарантуємо мінімальний кут, не вертикально
        if (b.homing && Math.abs(b.dx) < 1.2) {
            b.dx = 1.2 * (b.dx >= 0 ? 1 : -1);
        }
